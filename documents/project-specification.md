...
The advantages of Rust over C/C++ could significantly reduce the risks related to memory (segmentation fault, buffer overflow), keeping the flexibility of declaring variables on the heap or stack. Embedded systems are sometime not remplacable, for example a space probe sent to an asteroid, or an aircraft computer, and then are extremely sensitive to dysfunctions.
